...
namespace libutt {
        ...
        const RegAuthPK& rpk)
    {
         ...
         *  - Otherwise, if you can get servers to sign two different messages with the same H_j, you can forge a new signature as: 
        ...
        if(isBudgeted) {
            ...
            budget_pi = BudgetProof(
                forMeOutputs,
                p.getRegCK(),
                rcm,
                pid_hash_sender, ask.s, a,
                cks, icms, ts
            );
    ...
    bool Tx::quickPayValidate(const Params& p, const RandSigPK& bpk, const RegAuthPK& rpk) const {
        ...
        for(size_t i = 0; i < ins.size(); i++) {
            ...
            if(!ins[i].pi.verify(p, ins[i].null, rcm, ins[i].ccm, ins[i].vcm, txOutsHash)) {
                logerror << "txin[" << i << "] with a '" 
                    << Coin::typeToString(ins[i].coin_type)
    ...
    RandSigShare Tx::shareSignCoin(size_t txoIdx, const RandSigShareSK& bskShare) const {
        ...
        return bskShare.shareSign(
            getCommVector(txoIdx, H),
            H
        );
    }

    std::optional<Coin> Tx::tryClaimCoin(
        const Params& p,
        size_t txoIdx,
        const AddrSK& ask,
        size_t n,
