...
std::unique_ptr<TestSetup> TestSetup::ParseArgs(int argc, char *argv[], log4cplus::Logger logger)
{
    ...
    client_config->set(BCB::common::utt_wallet_file_key, wallet_file_prefix);
    ...
    std::string key_file_name = keys_file_prefix + 
                                    std::to_string(client_config->numReplicas);
