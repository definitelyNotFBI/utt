...
logging::Logger protocol::logger = logging::getLogger("sharding.bcb.replica");

protocol::protocol(asio::io_context& io_ctx, uint16_t port_num, 
                    std::shared_ptr<Cryptosystem> crypsys)
                : m_io_ctx_(io_ctx), m_acceptor_(io_ctx, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port_num)), metrics_timer(io_ctx), m_cryp_sys_{std::move(crypsys)}
{
    auto replicaConfig = ReplicaConfig::Get();
    using concord::storage::rocksdb::NativeClient;
    auto db_file = "utt-db-s"+std::to_string(replicaConfig->shard_id) + "-" + std::to_string(replicaConfig->getid());
