...
const auto COMM_CONFIG_ERROR = "invalid argument for --network-config-file-prefix";
...
std::unique_ptr<TestSetup> TestSetup::ParseArgs(int argc, char *argv[])
{
    ...
    std::string certRootPathPrefix = "certs";
    std::string utt_params_file;
    replica_config->numOfExternalClients = 80;
    std::optional<size_t> shard_id_field = std::nullopt;
    std::optional<size_t> num_shard_field = std::nullopt;

    auto logger = logging::getLogger("sharding.replica.setup");
                ...
                argc, argv, shortOptions, longOptions, &optionIndex)) != EOF) 
    {
        ...
        case 's': {
            shard_id_field = concord::util::to<size_t>(std::string{optarg});
        } break;
        case 'S': {
            num_shard_field = concord::util::to<size_t>(std::string{optarg});
        } break;
        case '?': {
    ...
    if (!shard_id_field.has_value()) {
        throw std::runtime_error("missing --shard-id");
    }
    replica_config->shard_id = shard_id_field.value();
    if (!num_shard_field.has_value()) {
        throw std::runtime_error("missing --num-shard");
    }
    replica_config->num_shards = num_shard_field.value();
    if (commConfigFilePrefix.empty()) {
        throw std::runtime_error("missing --comm-config-prefix");
    }
    ...
    // TODO: For testing, all shards have the same private/public key
    // Fixing this requires a re-write of Alin's wallet class
    replica_config->set(utt_bft::UTT_PARAMS_REPLICA_KEY, utt_params_file);

    // Get this replica's key
    std::string key_file_name = keysFilePrefix + std::to_string(replica_config->id) + "-s" + std::to_string(replica_config->shard_id);
    auto sys = getKeyFile(key_file_name);
    assert(sys != nullptr);
    auto crypsys = std::shared_ptr<Cryptosystem>(sys);
    // Setup rsa signer
    replica_config->rsa_signer.reset(new bftEngine::impl::RSASigner(replica_config->replicaPrivateKey.c_str()));

    // Get all replicas public keys
    typedef bftEngine::impl::RSAVerifier Verifier;
    std::vector<std::unordered_map<uint16_t, std::shared_ptr<Verifier>>> cryps_vec;
    for(size_t shard=0;shard<replica_config->num_shards;shard++) {
      std::string key_file_name =
          keysFilePrefix + std::to_string(replica_config->numReplicas) + "-s" + std::to_string(shard);
    ...
    replica_config->setpk_map(map);

    // Get communication config
    std::string commConfigFile = commConfigFilePrefix + std::to_string(replica_config->shard_id);
    std::string certRootPath = certRootPathPrefix + std::to_string(replica_config->shard_id);
    ...
    return std::make_unique<TestSetup>(logPropsFile, conf, std::move(crypsys));
}
