...
class SimpleAppState : public IRequestsHandler {
 ...
 public:
  SimpleAppState(uint16_t numCl, uint16_t numRep, libutt::Params p, libutt::BankShareSK bsk, libutt::BankPK bpk)
      : p(p), shareSK(bsk), bpk(bpk), statePtr{new SimpleAppState::State[numCl]}, numOfClients{numCl}, numOfReplicas{numRep} {}
...
class SimpleTestReplica {
 ...
 public:
  ...
  static SimpleTestReplica *create_replica(ISimpleTestReplicaBehavior *behv,
    ...
    libutt::Params p(fin);
    libutt::BankShareSK shareSK(fin);
    libutt::BankPK bpk(fin);
    // libutt::BankPK
    // fin >> simpleAppState->p;
    // fin >> simpleAppState->shareSK;
    // fin >> simpleAppState->bpk;

    LOG_DEBUG(replicaLogger, "Params:" << p);
    LOG_DEBUG(replicaLogger, "Params:" << shareSK);
    LOG_DEBUG(replicaLogger, "Params:" << bpk);

    // This is the state machine that the replica will drive.
    SimpleAppState *simpleAppState = new SimpleAppState(rp.numOfClients, rp.numOfReplicas, p, shareSK, bpk);
