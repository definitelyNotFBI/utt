...
std::unique_ptr<TestSetup> TestSetup::ParseArgs(int argc, char** argv) {
  ...
  try {
    ...
    replicaConfig.maxExternalMessageSize = 30*1024*10* replicaConfig.clientBatchingMaxMsgsNbr;
    ...
    static struct option longOptions[] = {
      ...
      {"consensus-batching-max-reqs-size", 
      ...
      {"preexecution-concurrency-level", 
                                        required_argument, 0, 'Y'},
      {"consensus-batching-flush-period", 
                                        required_argument, 0, 'z'},
      {0, 0, 0, 0}
    };
    int o = 0;
    int optionIndex = 0;
